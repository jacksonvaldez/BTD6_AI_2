...
class NeuralNetwork:
    ...
    def position(self, layer): # Randomly choose from the top 5 chosen positions
        numbers_x, numbers_y = np.split(layer, (layer.size // 2,))
        numbers_x = numbers_x.reshape(128,)
        numbers_y = numbers_y.reshape(128,)

        if len(numbers_x) == 0 or len(numbers_y) == 0:
            raise ValueError("Both input lists must contain at least one number")

        # Normalize the input numbers to be within [0, 1] range
        normalized_numbers_x = (numbers_x - np.min(numbers_x)) / (np.max(numbers_x) - np.min(numbers_x))
        normalized_numbers_y = (numbers_y - np.min(numbers_y)) / (np.max(numbers_y) - np.min(numbers_y))

        # Compute x and y coordinates using NumPy operations
        indices_x = np.arange(len(normalized_numbers_x))
        indices_y = np.arange(len(normalized_numbers_y))
        x = int(np.sum(indices_x * normalized_numbers_x) % 405)
        y = int(np.sum(indices_y * normalized_numbers_y) % 270)

        return (int(x), int(y))

    def tower(self, layer):
        return layer.argmax()

    def upgrade_path(self, layer):
        return layer.argmax()

    def action(self, layer):
        # 0: Place Tower
        # 1: Upgrade Tower
        # 2: Sell Tower
        # 3: Do Nothing
    ...
    def query_ann(self, layer_2): # ann - artifical/vanilla neural network
        ...
        layer_3 = layer_3.reshape(128, 1) + self.biases3
